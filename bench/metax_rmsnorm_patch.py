...
安装方式 (镜像内, 不修改 sglang 任何源文件, 规避 .py/.pyc 不一致问题):
  1. 本文件放入 /opt/conda/lib/python3.10/site-packages/
...
状态: 已本地静态审查; 部署验证因 muxi-01 临时失联待执行
      (见 docs/metax_c550_integration.md §9 部署 runbook)。
"""
...
def _apply_patch(module):
    ...
    if getattr(module, _PATCHED_FLAG, False):
        return
    ns = {}
    exec(_TRITON_KERNELS, ns)  # 此处才 import torch/triton (启动懒加载)
    torch = ns["torch"]
    ...
    def patched_rmsnorm(input, weight, eps=1e-6, out=None, enable_pdl=False):
        if ns["_ok"](input.shape[-1], input.dtype):
            n = input.shape[-1]
            x2 = input if input.dim() == 2 else input.contiguous().reshape(-1, n)
            y = torch.empty_like(x2)
            block, warps = ns["_cfg"](n)
            ns["_rmsnorm_kernel"][(x2.numel() // n,)](
                x2, weight, y, x2.stride(0), n, eps,
    ...
    def patched_fused(x, residual, weight, eps):
        if ns["_ok"](x.shape[-1], x.dtype):
            n = x.shape[-1]
            block, warps = ns["_cfg"](n)
            ns["_fused_add_rmsnorm_kernel"][(x.numel() // n,)](
                x, residual, weight, x.stride(0), n, eps,
                BLOCK=block, num_warps=warps)
            return x, residual
