...
class H200_SXM(Arch):
    def __init__(self):
        ...
        self.l2_bandwidth = 11164.159310e9 # 96* self.max_freq * 2 * 32 # no compression, 2 sectors/cycle?, 95???
        ...
        # L1.5: passive per-group cache, ~8 SMs share one L1.5
        # From microbench CSV: peak BW 12,758 GB/s at 1MB, plateau ~9,200 GB/s at 3MB+
        self.l1_5_group_size = 8
        self.l1_5_capacity_per_group = 180 * 1024  # ~180KB per group
        self.l1_5_bandwidth = 12758e9              # 12,758 GB/s peak from CSV
        self.l1_5_associativity = 8
        self.l1_5_cacheline_bytes = 128
        self.l1_5_max_util = 0.9

        # H200 与 H100 同一代 Hopper silicon → MMA 能力相同
        self.support_utcmma = False
        self.support_wgmma = True
        self.ddr_wave_bytes = 8196

    def set_to_spec(self):
        self

    def set_to_microbench(self):
        self.base_freq = 1.73 * 1e9
        self.max_freq = 1.73 * 1e9
        # self.ddr_max_util=0.9
        # self.l2_max_util=0.9
        # self.l1_max_util=0.9
        # self.compute_max_util=0.9
        ...
        self.fp16_tensor_flops = 927.754 * 1e12
    ...
    def get_tensor_core_minimum_ptx(self, bytes = 2):
        # 与 H100 一致 (Hopper wgmma 最小 PTX shape)
        if bytes == 2:
            return (8, 8, 16)
        elif bytes == 4:
            return (8, 8, 8)
