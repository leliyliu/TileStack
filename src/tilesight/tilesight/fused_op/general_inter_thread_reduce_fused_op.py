...
def calculate_general_ruduce_inter_thread_resource_utilization(out_shape,reduction_shape,out_axis_mapping,reduction_axis_mapping,out_tb_shape,dim_threads,reduce_threads_info,bytes_per_num, arch,mem_levels,compute_at=1):
# ret=calculate_N_N_elementwise_resource_utilization([20480,20480],[10,160],[10,10], 4,chip,conv_levels)
    ...
    l2_hit_rate=general_reduce_l2_hitrate(out_axis_mapping, out_tb_shape, spatial_grids, in1_tb_spatial_shape, in2_tb_spatial_shape, in1_tb_reduction_shape, in2_tb_reduction_shape,arch.l2_capacity,arch.sm_count,bytes_per_num,mem_levels)
    ...
    l2_read_io=bytes_per_num*np.prod(spatial_grids)*np.prod(reduction_grids)*(
        np.prod(in1_tb_spatial_shape)*np.prod(in1_tb_current_step_shape)*in1_level[0]+
        np.prod(in2_tb_spatial_shape)*np.prod(in2_tb_current_step_shape)*in2_level[0])
    
    l2_store_io=np.prod(spatial_grids)*np.prod(out_tb_shape)*bytes_per_num*out1_level[0]
    ...
    smem_footprint=(np.prod(in1_tb_spatial_shape)*np.prod(in1_tb_current_step_shape)*(1-(in1_level[1]-in1_level[0]))+
                    np.prod(in2_tb_spatial_shape)*np.prod(in2_tb_current_step_shape)*(1-(in2_level[1]-in2_level[0]))+
                    np.prod(out_tb_shape)*(out1_level[1]-out1_level[0]))*bytes_per_num
    # load in1 and in2, ldg
    smem_l1_io=bytes_per_num*np.prod(spatial_grids)*np.prod(reduction_grids)*(
        np.prod(in1_tb_spatial_shape)*np.prod(in1_tb_current_step_shape)*in1_level[0]+
        np.prod(in2_tb_spatial_shape)*np.prod(in2_tb_current_step_shape)*in2_level[0])
    ...
    if(compute_at==-1): # all data from smem
        reg_footprint=np.prod(out_thread_shape)+2+len(reduction_shape)
        smem_l1_io+=bytes_per_num*np.prod(spatial_grids)*np.prod(reduction_grids)*thread_per_tb*np.prod(out_thread_shape)*np.prod(reduction_step)*(in1_level[1]+in2_level[1])
        ...
        smem_l1_io+=bytes_per_num*np.prod(spatial_grids)*np.prod(reduction_grids)*thread_per_tb*np.prod(reduction_step)*(
            np.prod(in1_thread_shape)*in1_level[1]+np.prod(in2_thread_shape)*in2_level[1])
    ...
    smem_l1_io+=(np.prod(spatial_grids)*thread_per_tb*np.prod(out_thread_shape)*out1_level[1])*bytes_per_num/reduce_threads_info[1]
    ...
    if(reduce_threads_info[1]>32):
        smem_footprint+=128*bytes_per_num
        current_ruduce=reduce_threads_info[1]
        # smem_l1_io+=np.prod(spatial_grids)*bytes_per_num
        smem_l1_io+=np.prod(spatial_grids)*thread_per_tb/reduce_threads_info[1]*np.prod(out_thread_shape)*bytes_per_num*current_ruduce
        
        while(current_ruduce>32):
            smem_l1_io+=np.prod(spatial_grids)*thread_per_tb/reduce_threads_info[1]*np.prod(out_thread_shape)*bytes_per_num*current_ruduce*3/2
            current_ruduce/=2
        ...
        smem_l1_io+=math.log2(32)*np.prod(spatial_grids)*thread_per_tb/reduce_threads_info[1]*np.prod(out_thread_shape)*bytes_per_num*32*3

        # load shared and stg.e to global (repeated)
        smem_l1_io+=np.prod(spatial_grids)*reduce_threads_info[1]*np.prod(out_thread_shape)*bytes_per_num*2
