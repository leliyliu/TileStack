...
def calculate_N_0_general_ruduce_inter_thread_resource_utilization(out_shape,reduction_shape,out_axis_mapping,reduction_axis_mapping,out_tb_shape,dim_threads,reduce_threads_info,bytes_per_num, arch,mem_levels,compute_at=1):
# ret=calculate_N_N_elementwise_resource_utilization([20480,20480],[10,160],[10,10], 4,chip,conv_levels)
    ...
    if(compute_at==-1): # all data from smem
        ...
        smem_l1_io+=bytes_per_num*np.prod(spatial_grids)*np.prod(reduction_grids)*thread_per_tb*np.prod(out_thread_shape)*np.prod(reduction_step)*(in1_level[1])
        ...
        reg_footprint=np.prod(out_thread_shape)+np.prod(in1_thread_shape*in1_level[1])+len(reduction_shape)+2
