...
def calculate_row_reduce_resource_utilization(m, k, tb_m,m_thread, rstep, reduce_thread,  bytes_per_num, arch,mem_levels):
    ...
    overall_time = compute_time * m / arch.sm_count / tb_m 

    l2_read_avg = arch.sm_count * tb_m * k * bytes_per_num * in1_level[0]
    l2_st_avg = arch.sm_count * tb_m * 1 * bytes_per_num * out1_level[0]
    ...
    if (arch.core=="A100" or arch.core=="H100" or arch.core=="B200"):
        ...
        l2_bw_req = (l2_read_avg + l2_st_avg) / compute_time
    ...
    if reduce_thread <= 32:
        l1_io_avg = k_outer * tb_m * rstep * bytes_per_num * in1_level[0]
        smem_io_avg = k_outer * tb_m * rstep * bytes_per_num * in1_level[0]
        smem_io_avg += tb_m * k_outer * k_inner_outer * reduce_thread * bytes_per_num * max(tb_m // 4, 1)*in1_level[1]
        l1_io_avg += tb_m * bytes_per_num * reduce_thread*out1_level[1]
        ...
        while current_reduce > 32:
            smem_io_avg += current_reduce * 3 / 2 * bytes_per_num * 2 *in1_level[1]
            current_reduce //= 2
        smem_io_avg += math.ceil(math.log2(min(32, reduce_thread))) * 3 * 32 * bytes_per_num *in1_level[1]
        l1_io_avg += tb_m * bytes_per_num * reduce_thread *out1_level[1]
        smem_io_avg += l1_io_avg
        smem_io_avg *= SMEM_Conflict_para
    ...
    smem_bw_req = smem_io_avg * arch.sm_count / compute_time
