...
def calculate_N_1_elementwise_resource_utilization(in1_shape,in2_shape,in1_tb_shape,dim_threads,bytes_per_num, arch,mem_levels):
    ...
    l2_read_io=(np.prod(in1_shape)*in1_level[0]+np.prod(grids)*np.prod(in2_tb_shape)*in2_level[0])*bytes_per_num
    l2_store_io=np.prod(in1_shape)*bytes_per_num*out1_level[0]
    # print("in2_tb_shape",in2_tb_shape)
    # print("grids",grids)
    # print("in2_level",in2_level)
    # print("l2_read_io",l2_read_io)
    ddr_read_io=(np.prod(in1_shape)*in1_level[0]+np.prod(in2_shape)*in2_level[0])*bytes_per_num
    ddr_store_io=np.prod(in1_shape)*bytes_per_num*out1_level[0]
    ...
    smem_footprint=(np.prod(in2_tb_shape)*(1-(in2_level[1]-in2_level[0]))+
                           np.prod(in1_tb_shape)*(out1_level[1]-out1_level[0]))*bytes_per_num
    # load in1
    smem_l1_io=(np.prod(in1_shape)*in1_level[1])*bytes_per_num
    # load in2 , ldg + sts
    smem_l1_io+=(np.prod(grids)*np.prod(in2_tb_shape)*in2_level[0])*bytes_per_num*2
    # load in2 , lds
    smem_l1_io+=(np.prod(grids)*thread_per_tb*np.prod(in2_thread_shape)*in2_level[1])*bytes_per_num
    # smem_l1_io=(np.prod(in1_shape)*in1_level[1]+np.prod(in1_shape)*in2_level[1])*bytes_per_num
    smem_l1_io+=(np.prod(in1_shape)*out1_level[1])*bytes_per_num

    # 3 means 2 input + 1 ouput 
    reg_footprint=math.ceil((np.prod(in1_thread_shape)*(in1_level[1]+1)+np.prod(in2_thread_shape)*in2_level[1])*REG_spill_para)
    ...
    compute_flops=2*np.prod(in1_shape)*overheads
