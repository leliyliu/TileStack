...
def calculate_N_N_elementwise_resource_utilization(op_shape,tb_shape,dim_threads,bytes_per_num, arch,mem_levels):
    ...
    l2_read_io=(np.prod(op_shape)*in1_level[0]+np.prod(op_shape)*in2_level[0])*bytes_per_num
    ...
    smem_l1_io=(np.prod(op_shape)*in1_level[1]+np.prod(op_shape)*in2_level[1])*bytes_per_num
    smem_l1_io+=(np.prod(op_shape)*out1_level[1])*bytes_per_num

    # 3 means 2 input + 1 ouput 
    reg_footprint=math.ceil(np.prod(thread_tiles)*(1+in1_level[1]+in2_level[1])*REG_spill_para)
